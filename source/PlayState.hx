...
import LuaClass.LuaCamera; // Not a real error :)
...
class PlayState extends MusicBeatState
{
	...
	public var camZooming:Bool = false; // making public because i don't know
	...
	override public function create()
	{
		...
		camMovement = FlxTween.tween(this, {}, 0); // Note tween shit
		...
		dad = new Character(228, 309, SONG.player2);
		...
		if (dad.frames == null)
		{
			...
			FlxG.log.warn(["Couldn't load opponent: " + SONG.player2 + ". Loading default opponent"]);
			#end
			dad = new Character(228, 309, 'prymer');
		}
		...
		if(FlxG.save.data.colour)
        {
         switch (SONG.player2)
           {
             case 'prymer':
			  healthBar.createFilledBar(0xFFFF0000, 0xFF0097C4); // change to the right colors when the icons are done
            }
        }
        else
		...
		judgementCounter = new FlxText(20, 0, 0, "", 20); // Straight up stole from hex then edited it, sorry Kade :(
		judgementCounter.setFormat(Paths.font("vcr.ttf"), 20, FlxColor.WHITE, FlxTextAlign.LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		judgementCounter.borderSize = 2;
		judgementCounter.borderQuality = 2;
		judgementCounter.scrollFactor.set();
		judgementCounter.cameras = [camHUD];
		judgementCounter.y = 630;
		...
		if (isStoryMode)
		{
			...
			{
				case 'joyful': // This case goes unused
					startCountdown();
				default:
					startCountdown();
			}
		...
		if (generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				if (!daNote.mustPress && daNote.wasGoodHit)
				{
					...
					if (camFocus == 'dad')
					{
						triggerCamMovement(Math.abs(daNote.noteData % 4)); // Note tween shit
					}
	...
	private function popUpScore(daNote:Note):Void
	{
		...
		switch (daRating)
		{
			...
			var sploosh:FlxSprite = new FlxSprite(daNote.x, playerStrums.members[daNote.noteData].y); // note splashes
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void
	{
		if (!boyfriend.stunned)
		{
			...
			if (camFocus == 'bf')
				{
					triggerCamMovement(direction % 4); // Note tween shit
				}
	...
	function goodNoteHit(note:Note, resetMashViolation = true):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			if (camFocus == 'bf')
				{
					triggerCamMovement(note.noteData % 4); // Note tween shit
				}
	...
	override function stepHit()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			case 'joyful':
				switch (curStep)
				{
					case 0 | 0: // This case goes unused
						Stage.camZoom = 0.0;
				}
		}
